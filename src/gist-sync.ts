...
> = Effect.gen(function* () {
  ...
  const result = yield* Option.match(existingGistId, {
    onNone: () =>
      Effect.gen(function* () {
        const response = yield* GitHubApiService.createGist(token, files, workspace.name);
    ...
    onSome: (gistId) =>
      Effect.gen(function* () {
        const response = yield* GitHubApiService.updateGist(token, gistId, files, workspace.name);
        return {
          fileCount: Object.keys(files).length,
          gistId: response.id,
          gistUrl: response.html_url,
