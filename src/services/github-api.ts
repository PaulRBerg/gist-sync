...
export class GitHubApiService extends Effect.Service<GitHubApiService>()("GitHubApiService", {
  ...
  effect: Effect.gen(function* () {
    return {
      createGist: (token: string, files: FileContents, workspaceName: string) =>
        withTimeout(
          Effect.gen(function* () {
            const response = yield* fetchWithAbort(GIST_API_BASE, {
              body: JSON.stringify({
                description: buildGistDescription(workspaceName),
                files,
                public: false,
              }),
      ...
      updateGist: (token: string, gistId: string, files: FileContents, workspaceName: string) =>
        withTimeout(
          Effect.gen(function* () {
